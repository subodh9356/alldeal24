...
const ManageBikes = () => {
  ...
  useEffect(() => {
    ...
    onValue(bikesRef, (snapshot) => {
      ...
      if (data) {
        const bikesArray = Object.entries(data).map(([id, bike]) => ({
          id,
          ...bike,
        }));
        setBikes(bikesArray);
  ...
  const applyFilter = () => {
    ...
    const filtered = bikes.filter((bike) => {
      const postDate = new Date(bike.postedTime);
  ...
  const handleDelete = (bikeId) => {
    ...
    if (confirmDelete) {
      remove(ref(database, `Bikes/${bikeId}`));
    }
  };
      ...
      {bikes.length === 0 ? (
        ...
        <>
          <div className="property-header">
            <h2 className="property-title">Manage Bikes</h2>
            <span className="property-count">Total: {filteredBikes.length}</span>
          </div>
          ...
          <div className="card-grid">
            {filteredBikes.map((bike) => (
              <div key={bike.id} className="card-hover" style={styles.card}>
                <img src={bike.bikeImages?.[0]} alt={bike.bikeModel} style={styles.image} />
                <h3 style={styles.cardTitle}>{bike.bikeModel}</h3>
                <div style={styles.cardLine}><FaMapMarkerAlt style={styles.icon} /> {bike.bikeLocation}</div>
                <div style={styles.cardLine}><FaRupeeSign style={styles.icon} /> ₹{bike.bikePrice}</div>
                <button className="delete-button" onClick={() => handleDelete(bike.id)}>
                  <FaTrash /> Delete
                </button>
              </div>
            ))}
